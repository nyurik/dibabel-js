...
from .DataTypes import TitleSitelinksCache, SiteMetadata
...
class PagePrimary:
    ...
    def set_history(self, history: List[RevComment], metadata: SiteMetadata):
        self.history = []
        self.historic_dependencies = set()
        self.add_to_history(history, metadata)

    def add_to_history(self, history: List[RevComment], metadata: SiteMetadata):
        # assume history is going from oldest to newest
        # assume the data is already de-duplicated
        for rev in history:
            self.history.append(rev)
            self.dependencies = set(self.parse_dependencies(rev.content, metadata))
            self.historic_dependencies.update(self.dependencies)
    ...
    def compute_sync_info(self, qid: str, page: PageContent, metadata: SiteMetadata) -> SyncInfo:
        ...
        for hist in reversed(self.history):
            adj = self.localize_content(hist.content, metadata, page.domain)
    ...
    def parse_dependencies(self, content, metadata: SiteMetadata) -> Iterable[str]:
        if self.is_module:
            ...
            return (to_template(v[1]) for v in reTemplateName.findall(content)
                    if not metadata.is_magic_keyword(v[1]))

    def localize_content(self, content, metadata: SiteMetadata, target_domain: str) -> str:
        if self.is_module:
            return self.replace_module_deps(content, target_domain, ignore_modules)
        else:
            return self.replace_template_deps(content, target_domain, metadata.is_magic_keyword)
    ...
    def replace_module_deps(self,
        ...
        def sub_module(m):
            ...
            obj = self.title_sitelinks[fullname]
            try:
    ...
    def replace_template_deps(self,
        ...
        def sub_template(m):
            ...
            if ignore(name):
                return m.group(0)
            obj = self.title_sitelinks[to_template(name)]
            try:
